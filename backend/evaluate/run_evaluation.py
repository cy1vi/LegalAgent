...
class LegalEvaluator:
    ...
    def run(self):
        ...
        try:
            # 测试连接
            test_response = requests.post(batch_url, json={"facts": ["test"], "top_k": 1})
        ...
        for i in tqdm(range(0, len(self.dataset), BATCH_SIZE), desc=f"{self.eval_type} evaluation"):
            ...
            try:
                resp = requests.post(batch_url, json={"facts": batch_facts, "top_k": 20})
            ...
            for idx, raw_results in enumerate(batch_results):
                ...
                for res in raw_results:
                    if self._get_text_hash(res.get('fact', '')) == query_hash:
                        continue
                ...
                for pos in target_positives:
                    ...
                    if h: gold_hashes.add(h)

                for k in self.k_list:
                    if k > len(filtered_results):
                        continue  # 如果k大于实际返回结果数，跳过
                    ...
                    for res in current_results:
                        ...
                        if is_acc: hits['accusation'] += 1
                        if is_art: hits['article'] += 1
                        if is_imp: hits['imprisonment'] += 1
                    
                    for task in self.tasks:
                        h_count = hits[task]
                        
                        # 1. Macro Metrics (Per Query)
