...
def evaluate_crime_centroids(
    ...
    # 筛选出单罪名案例
    print("🔍 Filtering single-crime cases...")
    single_crime_indices = []
    for i, accusations in enumerate(all_accusations):
        if len(accusations) == 1:
            single_crime_indices.append(i)
    
    print(f"✅ Found {len(single_crime_indices)} single-crime cases out of {N} total cases.")
    
    # 随机抽样（仅从单罪名案例中抽样）
    random.seed(seed)
    np.random.seed(seed)
    sample_count = min(sample_size, len(single_crime_indices))
    selected_indices = np.random.choice(single_crime_indices, size=sample_count, replace=False)
    print(f"🎲 Selected {len(selected_indices)} single-crime samples for evaluation.")
    ...
    for idx in tqdm(selected_indices, desc="Evaluating"):
        ...
        # 由于我们只选择了单罪名案例，这里可以确保 len(true_crimes) == 1
        if not true_crimes:
            continue
        total_valid += 1
...
if __name__ == "__main__":
    ...
    CENTROID_PATH = r"F:\LegalAgent\backend\data\fact_embeddings\single_crime_centroids.pkl"
